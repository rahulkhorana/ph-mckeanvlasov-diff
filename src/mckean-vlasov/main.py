...
from models import (
    ...
    time_embed as time_embed_fn,  # only used for UNet init
)
from losses_steps import (
    DiffusionState,
    create_diffusion_state,
    diffusion_train_step,  # uses Charbonnier + SNR weighting (your file)
)
from sampling import mv_sde_sample  # prob_flow_ode will be forced True below
...
def build_mods_embedder(enc: ModulesTrajectoryEncoder):
    def mods_embed_fn(params, mods_batch: List[List]):
        ...
        for mods in mods_batch:
            ...
            # f: (1,1,S,F) -> (1,S,F), s: (1,1,S,1) -> (1,S,1), t: (1,1,1) -> (1,1)
            f = jnp.squeeze(f, axis=0)
            s = jnp.squeeze(s, axis=0)
            t = jnp.squeeze(t, axis=0)
            feats_list.append(f)  # (1,S,F)
            set_list.append(s)  # (1,S,1)
            time_list.append(t)  # (1,1)

        # Stack to (B,1,S,F), (B,1,S,1), (B,1,1) which matches encoder signature (B,T,S,F) with T=1
...
# Simple fixed (non-trainable) label embedding table
def init_label_proj(rng, num_classes: int, y_dim: int) -> jnp.ndarray:
    k, _ = jax.random.split(rng)
    table = jax.random.normal(k, (num_classes, y_dim), dtype=jnp.float32) * (
        1.0 / np.sqrt(y_dim)
    )
    return table
...
def save_ckpt(obj, path: Path):
    ...
    path.write_bytes(serialization.to_bytes(obj))
...
def main():
    ...
    ap.add_argument(
        "--schedule", type=str, choices=["cosine", "linear"], default="cosine"
    )
    ...
    ap.add_argument(
        "--cfg_sched", type=str, choices=["linear", "cosine", "exp"], default="cosine"
    )
    ap.add_argument("--cfg_strength", type=float, default=5.0)

    # MV mean-field (applied only at sampling)
    ap.add_argument(
        "--mf_mode", type=str, choices=["none", "voxel", "rbf"], default="rbf"
    )
    ...
    ap.add_argument("--sample_label", type=int, default=-1)  # -1 -> use batch labels
    args = ap.parse_args()
    ...
    # robust normalization over train
    ...
    # UNet
    unet = UNet3D_FiLM(ch=48)  # slim to keep VRAM sane
    ...
    diff_state: DiffusionState = create_diffusion_state(
        rng=rng,
        apply_fn=unet.apply,
    ...
    for step in range(1, args.steps + 1):
        ...
        m_emb = mods_embed_fn(enc_params, batch["modules"])  # (B,256)
        m_emb = jnp.nan_to_num(m_emb, nan=0.0, posinf=0.0, neginf=0.0)  # type: ignore
        cond_vec = jnp.concatenate([y_emb, m_emb], axis=-1)  # (B, y_dim+256)
        ...
        # diffusion step (Charbonnier + SNR weighting inside)
        ...
        if step % 20 == 0:
            print(f"step {step:05d} | diff_loss={float(dloss):.4f}")
        ...
        if do_sample:
            ...
            m_emb_s = mods_embed_fn(enc_params, batch["modules"])
            ...
            # ALWAYS deterministic (prob_flow_ode=True) as requested
            samples = mv_sde_sample(
                unet_apply=diff_state.apply_fn,
                params=diff_state.ema_params,  # EMA params for sampling
                shape=(B, H, W, K, C),
                ...
                null_cond_vec=jnp.zeros_like(cond_vec_s),
                ...
                prob_flow_ode=True,  # <<< forced deterministic
                return_all=False,
            )

            # de-standardize to original scale and save + labels for metrics pairing
            samples_np = np.array(standardize_invert(samples, mu, sigma))  # type: ignore
            save_numpy(samples_np, gen_dir / f"samples_step{step:06d}.npy")
            save_numpy(
                np.asarray(y_idx, dtype=np.int32),
                gen_dir / f"labels_step{step:06d}.npy",
            )
