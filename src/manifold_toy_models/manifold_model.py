...
def make_loss_fn(static_model, manifold, sigma_fn):
    def loss_fn(params, x0, key):
        ...
        t = jax.random.uniform(key_t, (B,), minval=1e-3, maxval=1.0)  # (B,)
        z = jax.random.normal(key_noise, x0.shape)  # (B,D)
        ξ = jax.vmap(manifold.project_to_tangent)(x0, z)  # (B,D)

        xt = x0 + sigma_fn(t)[:, None] * ξ  # (B,D)
        xt = manifold.project(xt)  # (B,D)

        target = -ξ / sigma_fn(t)[:, None]  # (B,D)
